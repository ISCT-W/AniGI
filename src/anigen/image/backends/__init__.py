...
from ..config import DOTENV, gemini_key, image_model, gpt_key, gpt_image_model
...
BACKENDS = {
    ...
    "gemini": BackendSpec("gemini", "api", "Gemini 图片生成与编辑 REST 适配器", _gemini),
...
def resolve_model(name, explicit=None, *, env_file=DOTENV):
    ...
    if name == "gpt":
        return gpt_image_model(env_file=env_file)
