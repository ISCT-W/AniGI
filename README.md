...
本次更新减少重复参考和临时文件，并把任务首页改成逐轮阅读入口。完整实际 prompt、候选媒体、审查意见和修改理由仍保留；机器状态继续负责额度、恢复与审批关联。

| 变化 | 当前行为 |
| --- | --- |
| 图片输入去重 | 同一图片阶段的相同输入共用按内容寻址的不可变资产，各轮仍记录角色与输入顺序。 |
| 可读制作记录 | 视频每轮提供 prompt、参考绑定、设置、审查及修改理由；首页链接到报告。新首帧记录使用 Markdown，既有格式仍兼容。 |
| 命令参数 | 视频命令支持 `--input -` 从标准输入读取 JSON，无需为每次操作保存小参数文件。 |
| 完成任务精简 | `compact` 先严格核验已通过全片，再写入归档收据并清理抽帧、检查窗口、观察副本及可去重输入。保留准确审查版、参考和实质审查内容。 |
| 归档后的校验 | 核验保留资产和冻结审批；文件变化会使交付失效。中断后可以重复清理，不重置任何预算。 |

```sh
python3 run.py compact --task '<任务目录>'
```

**精简归档不是普通缓存清除。** 它保留历史审批，归档后可以查看、交付和记录用户验收，但制作入口会拒绝直接继续生成或重开审核。恢复证据或重新审查后才能继续制作；当前没有自动恢复缓存命令。它不代表重新审片，也不代表用户已接受成品。尚有未决生成、上传、观察或远端清理时不允许归档。目前 `compact` 仅支持已通过的视频任务。

不要在生成目录编写执行脚本或另存重复请求 JSON。观察原始响应等仍承载实质内容或恢复依据的记录不会按扩展名一律删除。细节见 [制作合同](docs/workflows.md#精简保存)。

本次验证：336 项合成离线测试通过，覆盖归档中断恢复、重复清理、保留媒体篡改、审批一致性和原有生成流程；未调用真实生成服务。

公开提交只包含允许清单中的源代码、文档与合成测试。`generation/`、真实参考和媒体、运行状态、本地配置及清理报告不进入 Git；发布前仍须检查实际跟踪列表、暂存内容与待推送历史，不能只依赖忽略规则。
