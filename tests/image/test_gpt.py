...
from anigen.image.backends.types import BackendError, ImageInput, ImageRequest
from anigen.image.config import gpt_key
from anigen.image.generation import check_request, generate_round
from anigen.image.task_store import TaskStore, StoreError
...
class GPTTests(unittest.TestCase):
    ...
    def test_frozen_fields_and_live_runner(self):
        with tempfile.TemporaryDirectory() as directory, patch('anigen.image.config.reference_project', return_value='fixture'):
            ...
            with patch('anigen.image.backends.gpt_key', side_effect=AssertionError('no key')):
                _, req = check_request(store, r)
            self.assertEqual(req.pixel_size, '1024x1024')
            with patch('anigen.image.backends.gpt_key', return_value='fake-key'), patch('anigen.image.backends.gpt._http_transport', self.transport):
