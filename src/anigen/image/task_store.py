...
def positive_limit(value):
    if type(value) is not int or not 1 <= value <= DEFAULT_REQUEST_LIMIT:
        raise StoreError("图片任务调用上限必须为 1 到 6 的整数")
...
class TaskStore:
    ...
    @classmethod
    def create(cls, path, brief, title, mode="offline", authorization="", limit=DEFAULT_REQUEST_LIMIT,
               project_id=None, reference_scope_id=None, initial_backend=None):
        require_text(brief, "用户原始请求")
        require_text(title, "任务名")
        positive_limit(limit)
        if initial_backend is not None and initial_backend not in {"gpt", "gemini"}:
            raise StoreError("初始图片后端须为 gpt 或 gemini")
        from .config import reference_project

        allowed_project = reference_project(mode)
        if reference_scope_id is not None:
            if project_id is not None and project_id != reference_scope_id:
                raise StoreError("参考范围参数冲突")
            project_id = reference_scope_id
        ...
        for name in ("reference.md", "feedback.md"):
            ...
            if not source.is_file():
                raise StoreError(f"缺少包内模板：{name}")
        ...
        with task._locked():
            ...
            if initial_backend is not None:
                state.update(initial_backend=initial_backend, selected_backend=initial_backend)
            task._save(state)
        return task
    ...
    def _remaining(state):
        ...
        total_used = sum(a["status"] not in UNCHARGED for a in state["attempts"])
        # Additional authorization records never replenish this task's budget.
        task_limit = min(DEFAULT_REQUEST_LIMIT, state["batches"][0]["limit"])
        return max(0, min(batch["limit"] - used, task_limit - total_used))
    ...
    def _check_inputs(self, state, attempt):
        ...
        if attempt.get("backend_authorization"):
            self._check(attempt["backend_authorization"])
    ...
    def prepare(self, prompt, reference, backend, model, inputs=(), parent=None,
                ...
                pixel_size=None, quality=None, backend_authorization=None, idempotency_key=None):
        ...
        if idempotency_key is not None:
            require_text(idempotency_key, "幂等标识")
            if len(idempotency_key) > 200 or any(ord(c) < 32 for c in idempotency_key):
                raise StoreError("幂等标识须为不含控制字符的短文本")
        fingerprint = digest(json.dumps({
            "prompt": prompt, "reference": reference, "backend": backend, "model": model,
            "inputs": [(role, digest(data), suffix) for role, data, suffix in source_images],
            "parent": parent, "submission": submission, "operation": operation,
            "aspect_ratio": aspect_ratio, "image_size": image_size, "pixel_size": pixel_size,
            "quality": quality, "backend_authorization": backend_authorization,
        }, sort_keys=True, ensure_ascii=False).encode())
        with self._locked():
            state = self._read()
            if idempotency_key is not None:
                existing_attempt = next((a for a in state["attempts"] if a.get("idempotency_key") == idempotency_key), None)
                if existing_attempt:
                    if existing_attempt.get("preparation_sha256") != fingerprint:
                        raise StoreError("同一幂等标识已绑定不同输入，禁止创建另一轮")
                    self._check_inputs(state, existing_attempt)
                    return existing_attempt["id"]
            if state["status"] != "active":
                raise StoreError("任务已暂停、结束或取消；先核对恢复范围")
            selected_backend = state.get("selected_backend")
            if selected_backend is None and state["attempts"]:
                selected_backend = state["attempts"][-1]["backend"]
            if selected_backend is not None and selected_backend != backend:
                require_text(backend_authorization, "任务内更换图片后端需要用户明确授权原话")
                if any(a["status"] in UNRESOLVED for a in state["attempts"]):
                    raise StoreError("先核对未决请求，不能换后端重发")
            self._check(state["brief"])
            parent_record = None
            if parent:
            ...
            if idempotency_key is not None:
                attempt.update(idempotency_key=idempotency_key, preparation_sha256=fingerprint)
            if backend_authorization is not None:
                require_text(backend_authorization, "更换后端授权")
                attempt["backend_authorization"] = self._file(f"{folder}/backend-authorization.md", backend_authorization.encode())
            if submission is not None:
                attempt["request"] = {
    ...
    def reserve(self, round_id, evidence_ready=False):
        with self._locked():
            ...
            if self._remaining(state) < 1:
                raise StoreError("已达到图片任务总调用上限或本批较低上限")
            self._check_inputs(state, attempt)
    ...
    def _valid_pass(self, state, attempt, candidate):
        if attempt["status"] != "succeeded":
            raise StoreError("通过候选必须来自已成功收录的轮次")
        self._check_inputs(state, attempt)
    ...
    def approved_candidate(self, round_id, candidate):
        """Return a verified immutable-file receipt without exporting the image.

        This validates recorded judgments and exact files; it does not perform
        semantic image review. Callers still enforce their own suitability gate.
        """
        with self._locked():
            state = self._read()
            attempt = self._attempt(state, round_id)
            ...
            return deepcopy({
                "task_id": state["id"], "round_id": round_id, "mode": state["mode"],
                "reference_scope_id": state["project_id"],
                "image": self._output(attempt, candidate), "review": review,
                "prompt": attempt["prompt"], "reference": attempt["reference"],
                "inputs": attempt["inputs"], "request": attempt.get("request"),
                "brief": state["brief"], "backend": attempt["backend"], "model": attempt["model"],
                "authorization": state["batches"][attempt["batch"] - 1]["authorization"],
                "backend_authorization": attempt.get("backend_authorization"),
            })

    def validate_pass(self, round_id, candidate):
        """Alias used by consumers that validate a pass without promoting it."""
        return self.approved_candidate(round_id, candidate)
    ...
    def abandon_prepared(self, round_id, note):
        """Cancel a confirmed unreserved draft, with no request or budget charge."""
        require_text(note, "取消未发送草稿的原因")
        with self._locked():
            state = self._read()
            attempt = self._attempt(state, round_id)
            if attempt.get("draft_cancellation"):
                if attempt["status"] != "not_sent" or attempt["draft_cancellation"] != note:
                    raise StoreError("已保存的草稿取消记录不一致")
                return deepcopy(attempt)
            if attempt["status"] != "prepared" or attempt.get("receipt") or attempt.get("outputs"):
                raise StoreError("只能取消确认尚未预留或发送的草稿；未知请求先核实")
            self._check_inputs(state, attempt)
            attempt["status"] = "not_sent"
            attempt["draft_cancellation"] = note
            attempt["events"].append({"at": now(), "status": "not_sent", "note": note, "kind": "draft_cancelled"})
            self._save(state)
            return deepcopy(attempt)

    def new_batch(self, authorization, limit=DEFAULT_REQUEST_LIMIT):
        """Record renewed authorization within the original lifetime budget."""
    ...
    def _warnings(self, state):
        ...
        for attempt in state["attempts"]:
            ...
            if attempt.get("backend_authorization"):
                records.append(attempt["backend_authorization"])
    ...
    def _render(self, state, warnings=None):
        ...
        lines = [f"# {state['title']}", "", mode, "",
                 ...
                 f"全任务已占用 {sum(a['status'] not in UNCHARGED for a in state['attempts'])} / {state['batches'][0]['limit']} 次；当前授权批次 {batch['id']}，剩余 {self._remaining(state)} 次。预留及未知请求保守计入，重开批次不恢复额度。", "",
    ...
    def _render_feedback(self, state):
        ...
        start, end = "<!-- anigen.image:acceptance:start -->", "<!-- anigen.image:acceptance:end -->"
