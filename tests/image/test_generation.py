...
from anigen.image.backends import make_backend, resolve_model
from anigen.image.backends.gemini import GeminiBackend
from anigen.image.backends.types import BackendError, ImageOutput, ImageResult
from anigen.image.cli import main
from anigen.image.config import gemini_key, read_settings
from anigen.image.generation import check_request, generate_round
...
class GenerationTests(unittest.TestCase):
    ...
    def prepare(self, backend="fixture-a", model="fixture", backend_authorization=None):
    ...
    def test_second_backend_uses_same_runner_and_shared_budget(self):
        ...
        generate_round(self.store, self.prepare("fixture-b", backend_authorization="Synthetic explicit backend change"),
                       execute=True, evidence_ready=True, backend=second)
    ...
    def test_validation_has_no_key_loading_or_network(self):
        ...
        with patch("anigen.image.backends.gemini_key", side_effect=AssertionError("key must not load")), \
             patch("anigen.image.backends.gemini._http_transport", side_effect=AssertionError("network forbidden")):
    ...
    def test_cli_limits_default_to_six_and_preserve_explicit_values(self):
        ...
        with redirect_stdout(io.StringIO()), patch("anigen.image.cli.generate_round", side_effect=AssertionError("no model calls")):
...
class ConfigTests(unittest.TestCase):
    ...
    def test_keys_are_parsed_without_shell_expansion_and_environment_wins(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "env-fake-key"}, clear=True):
