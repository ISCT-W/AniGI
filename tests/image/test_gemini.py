...
from anigen.image.backends import gemini  # noqa: E402
from anigen.image.backends.gemini import GeminiBackend  # noqa: E402
from anigen.image.backends.types import BackendError, ImageInput, ImageRequest  # noqa: E402
...
class GeminiTests(unittest.TestCase):
    ...
    def test_default_transport_installs_redirect_blocker_and_does_not_read_error(self):
        ...
        with mock.patch("anigen.image.backends.common.build_opener", return_value=opener) as build:
            backend = GeminiBackend(KEY)
            with self.assertRaises(BackendError) as caught:
                backend.generate(self.request)
        self.assertEqual(caught.exception.status, "failed")
        handler = build.call_args.args[0]
        self.assertIsInstance(handler, __import__("anigen.image.backends.common", fromlist=["_NoRedirect"])._NoRedirect)
