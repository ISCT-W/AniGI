...
def parser():
    p = argparse.ArgumentParser(description="AniGen 图片工作流；只有 generate --execute 会调用生成 API")
    ...
    init.add_argument("--limit", type=int, default=DEFAULT_REQUEST_LIMIT, help="全任务请求上限，最多 6；不会修改已有任务")
    ...
    prep.add_argument("--env-file", default=DOTENV)
    prep.add_argument("--input", nargs=2, action="append", metavar=("ROLE", "PATH"), default=[])
    prep.add_argument("--backend-authorization-file", help="用户明确更换后端的授权原话 Markdown")
    prep.add_argument("--idempotency-key", help="重复准备同一逻辑请求时使用稳定标识")
    ...
    batch = sub.add_parser("new-batch", help="记录继续授权；不恢复全任务额度")
    batch.add_argument("task")
    batch.add_argument("--authorization-file", required=True)
    batch.add_argument("--limit", type=int, default=DEFAULT_REQUEST_LIMIT, help="本批较低上限；不能增加任务总额度")
    ...
    execute.add_argument("--env-file", default=DOTENV)
        ...
        if args.command == "init":
            ...
            if args.command == "prepare":
                ...
                number = task.prepare(read_md(args.prompt_file), read_md(args.reference_file), args.backend,
                                      model, inputs=args.input, parent=args.parent,
                                      backend_authorization=read_md(args.backend_authorization_file) if args.backend_authorization_file else None,
                                      idempotency_key=args.idempotency_key, **extra)
                ...
                print(f"本任务剩余 {recovered['remaining']} 次；未决轮次：{', '.join(recovered['unresolved']) or '无'}。")
